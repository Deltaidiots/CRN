...
def map_pointcloud_to_image(
    ...
    # + renormalization).
    points = view_points(pc.points[:3, :], np.array(cam_calibrated_sensor["camera_intrinsic"]), normalize=True)
...
lidar_key = "LIDAR_TOP"
cam_keys = ["CAM_FRONT_LEFT", "CAM_FRONT", "CAM_FRONT_RIGHT", "CAM_BACK_RIGHT", "CAM_BACK", "CAM_BACK_LEFT"]
...
def worker(info):
    ...
    points = np.fromfile(os.path.join(data_root, lidar_path), dtype=np.float32, count=-1).reshape(-1, 5)[..., :4]
    lidar_calibrated_sensor = info["lidar_infos"][lidar_key]["calibrated_sensor"]
