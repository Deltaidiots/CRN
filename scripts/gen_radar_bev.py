...
# x, y, z, rcs, vx_comp, vy_comp, (dummy field for sweep info)
SAVE_FIELDS = [0, 1, 2, 5, 8, 9, -1]
...
def worker(info):
    """
    Processes and aggregates radar data from multiple sweeps for a given sample in the NuScenes dataset.
    
    This function transforms radar points from their sensor-specific frames to a vehicle-centric LiDAR reference frame.
    It accounts for the movement of both the vehicle and detected objects, ensuring the spatial accuracy of radar data.
    Data points are filtered based on predefined distance thresholds and saved for further processing.
    
    Args:
        info (dict): Contains metadata and file paths for radar and LiDAR data of a single NuScenes sample.
                     Key components include 'lidar_infos', 'sample_token', and paths to radar sweep files.
    
    Procedure:
    - Aggregates radar data points from the current and previous sweeps up to N_SWEEPS.
    - Transforms each point's coordinates and velocities into the vehicle's LiDAR reference frame.
    - Compensates for detected object movements and filters points based on distance thresholds.
    - Saves the processed points in a binary file for each radar sweep.
    
    Note:
    - Designed for parallel execution across dataset samples to efficiently process large volumes of radar data.
    - The transformation sequence involves multiple coordinate frames: sensor, vehicle (LiDAR-based), and global.
    
    Returns:
        None: The function saves processed radar data points to binary files without returning any value.
    """
    # Init.
    points = np.zeros((18, 0))
    all_pc = RadarPointCloud(points)

    sample = nusc.get('sample', info['lidar_infos']
                      ['LIDAR_TOP']['sample_token'])
    ...
    ref_cs_rec = nusc.get('calibrated_sensor',
                          lidar_data['calibrated_sensor']['token'])
    ref_time = 1e-6 * lidar_data['timestamp']
    # tranformation used to bring points from the vehicle frame back to the LiDAR sensor frame.
    ref_from_car = transform_matrix(ref_cs_rec['translation'],
                                    Quaternion(ref_cs_rec['rotation']), inverse=True)
    # This matrix converts points from the global frame (e.g., GPS coordinates) to the vehicle's reference frame, as established by the LiDAR sensor
    car_from_global = transform_matrix(ref_pose_rec['translation'],
                                       Quaternion(ref_pose_rec['rotation']), inverse=True)

    if DEBUG:
        lidar = LidarPointCloud.from_file(
    ...
    for chan in RADAR_CHAN:
        ...
        for sweep in range(N_SWEEPS):
            ...
            current_pc = RadarPointCloud.from_file(
                file_name, invalid_states, dynprop_states, ambig_states)

            # Get past pose. 
            current_pose_rec = nusc.get(
                'ego_pose', current_sd_rec['ego_pose_token'])
            # This matrix converts the points from the global frame to the vehicle's reference frame.
            global_from_car = transform_matrix(current_pose_rec['translation'],
                                               Quaternion(current_pose_rec['rotation']), inverse=False)

            # Get past calibrated sensor.
            current_cs_rec = nusc.get(
                'calibrated_sensor', current_sd_rec['calibrated_sensor_token'])
            # This matrix converts the points from the radar sensor frame at current sweep to the vehicle's reference frame.
            ...
            # transform is perfrom in reverse order i.e car_from_current -> global_from_car -> car_from_global -> ref_from_car
            ...
            velocities = np.vstack(
                (velocities, np.zeros(current_pc.points.shape[1])))
            velocities = np.dot(Quaternion(
                current_cs_rec['rotation']).rotation_matrix, velocities)
            velocities = np.dot(Quaternion(
                ref_cs_rec['rotation']).rotation_matrix.T, velocities)
